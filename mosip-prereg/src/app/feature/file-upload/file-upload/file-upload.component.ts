...
import { OcrService } from "../../ocr-integration/services/ocr.service";
import { OCR_TO_MOSIP_FIELD_MAP } from "src/app/shared/models/ocr-response.model";
...
export class FileUploadComponent implements OnInit, OnDestroy {
  ...
  onModification() {
    if (
      ...
      this.users[0].files.documentsMetaData.length > 0 &&
  ...
  /**
   * Handle OCR data extracted event
   * @param ocrData - Extracted data from OCR service
   */
  handleOcrDataExtracted(ocrData: any) {
    console.log('OCR Data Received:', ocrData);

    if (!this.users || !this.users[0]) {
      console.error('No user data available for OCR auto-fill');
      return;
    }

    const demographicDetails = this.users[0].request.demographicDetails;

    if (!demographicDetails || !demographicDetails.identity) {
      console.error('No demographic details available');
      return;
    }

    // Track how many fields were auto-filled
    let autoFilledCount = 0;

    // Map OCR fields to MOSIP fields and update
    Object.keys(ocrData).forEach(ocrField => {
      const mosipField = OCR_TO_MOSIP_FIELD_MAP[ocrField.toLowerCase()];

      if (mosipField && ocrData[ocrField].confidence >= this.ocrService.getConfidenceThreshold()) {
        this.updateDemographicField(
          demographicDetails.identity,
          mosipField,
          ocrData[ocrField].value
        );
        autoFilledCount++;
        console.log('Auto-filled ' + mosipField + ' with value: ' + ocrData[ocrField].value + ' (confidence: ' + ocrData[ocrField].confidence + ')');
      }
    });

    if (autoFilledCount > 0) {
      this.showOcrSuccessDialog(autoFilledCount);
    }
  }

  /**
   * Update demographic field with OCR value
   * @param identity - Identity object from demographic details
   * @param fieldName - MOSIP field name
   * @param value - Value to set
   */
  updateDemographicField(identity: any, fieldName: string, value: string) {
    // Handle multi-language fields
    if (Array.isArray(identity[fieldName])) {
      // Update all language versions with the same value
      identity[fieldName].forEach((langVersion: any) => {
        if (langVersion && typeof langVersion === 'object') {
          langVersion.value = value;
        }
      });
    } else {
      // Simple field - direct assignment
      identity[fieldName] = value;
    }
  }

  /**
   * Show OCR success dialog
   * @param count - Number of fields auto-filled
   */
  showOcrSuccessDialog(count: number) {
    const message = `Successfully auto-filled ${count} field(s) from the uploaded document!`;
