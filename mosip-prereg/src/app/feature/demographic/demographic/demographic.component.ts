...
// Commented out - package no longer available
// import {
//   MatKeyboardService,
//   MatKeyboardRef,
...
export class DemographicComponent
   ...
   */
  autoFillFromOCR() {
    const ocrData = this.ocrDataService.getMappedFormData();
    if (!ocrData || Object.keys(ocrData).length === 0) {
      console.log('No OCR data available for auto-fill');
      return;
    }

    console.log('Auto-filling demographic form with OCR data:', ocrData);
    console.log('Available form controls:', Object.keys(this.userForm.controls));
    console.log('Data capture languages:', this.dataCaptureLanguages);
    ...
    if (ocrData['dateOfBirth']) {
      ...
      try {
        ...
        let parsed = moment(dateStr, dateFormats, true);
        if (parsed.isValid()) {
      ...
      for (const dobCtrl of dobControls) {
        if (this.userForm.controls[dobCtrl]) {
          try {
            this.userForm.controls[dobCtrl].setValue(dateValue || dobValue);
            console.log(`✅ Set ${dobCtrl} = ${dateValue || dobValue}`);
            filledCount++;
            break;
          } catch (e) {
            console.log(`❌ Error setting ${dobCtrl}:`, e);
          }
        }
      }
    }

    // Handle gender specially (dropdown field)
    if (ocrData['gender']) {
      ...
      if (genderCode && this.userForm.controls['gender']) {
        try {
          this.userForm.controls['gender'].setValue(genderCode);
          console.log(`✅ Set gender = ${genderCode}`);
          filledCount++;
        } catch (e) {
          console.log(`❌ Error setting gender:`, e);
        }
      }
    }
  ...
  /**
   * Open OCR scanner in new window/popup
   */
  openOcrScanner() {
    ...
    const messageHandler = (event: MessageEvent) => {
      if (event.origin !== 'http://localhost:8001' && event.origin !== 'http://127.0.0.1:8001') {
        return;
      }

      if (event.data && event.data.type === 'OCR_DATA_EXTRACTED') {
        console.log('Received OCR data from popup:', event.data.payload);
  ...
  openKeyboard(controlName: string, langCode: string) {
    ...
    if (index > -1) {
      ...
      // Commented out - ngx7-material-keyboard package no longer available
