...
export class TimeSelectionComponent
  ...
  private prepareNameList(userInfo, regCenterInfo) {
    ...
    userInfo.forEach((user) => {
      ...
      // Safely get name field with fallback to 'fullName'
      const nameField = this.name || 'fullName';
      let nameValues = demographicData[nameField];

      // Handle case where name field might not be an array
      if (!nameValues || !Array.isArray(nameValues)) {
        nameList.fullName = nameValues || "User";
        ...
        if (!nameList.fullName && nameValues.length > 0) {
          nameList.fullName = nameValues[0].value || "User";
        }
      } else {
        nameList.fullName = (nameValues[0] && nameValues[0].value) || "User";
      }
  ...
  formatJson(centerDetails: any) {
    centerDetails.forEach((element) => {
      ...
      const ltrLangsStr = this.configService
        .getConfigByKey(appConstants.CONFIG_KEYS.mosip_left_to_right_orientation) || "eng,fra";
      const ltrLangs = ltrLangsStr.split(",");
      ...
      element.displayDay = this.DAYS ? this.DAYS[index] : ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"][index];
      if (!element.inActive) {
