...
export class AcknowledgementComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    let notificationTypesStr = this.configService
      .getConfigByKey(appConstants.CONFIG_KEYS.mosip_notification_type) || "EMAIL|SMS";
    let notificationTypes = notificationTypesStr.split("|");
      ...
      preRegIds.forEach(async (prid: any, index) => {
        await this.getUserDetails(prid).then(async (user) => {
          ...
          applicationLanguages.forEach(applicationLang => {
            ...
            // Use configured name field or fallback to 'fullName'
            const nameField = this.name || 'fullName';
            if (demographicData[nameField]) {
              ...
              if (Array.isArray(nameValues)) {
                ...
                // Fallback: if no matching language, use first value
                if (!nameListObj.fullName && nameValues.length > 0) {
                  nameListObj.fullName = nameValues[0]["value"] || "";
                }
              } else {
                nameListObj.fullName = String(nameValues);
              }
            }
